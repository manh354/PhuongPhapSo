...
def mainRungaKutta3_Heun(symbolic_function_system : list, symbolic_vars : list[sp.Symbol], symbolic_t : sp.Symbol , vars_start: list[float], t_start: float, t_end: float,h : float):
    ...
    while t_iterate <= t_end:
        equation_system_values_k1 = [equation((vars_iterate),t_iterate) for equation in lamdified_equation_system]
        vars_added_k1 = np.add(vars_iterate, np.multiply(1/3*h,equation_system_values_k1))
        equation_system_values_k2 = [equation((vars_added_k1),t_iterate + 1/3*h) for equation in lamdified_equation_system]
        vars_added_k2 = np.add(vars_iterate,np.multiply(2/3*h,equation_system_values_k2))
        equation_system_values_k3 = [equation((vars_added_k2),t_iterate + 2/3*h) for equation in lamdified_equation_system]
